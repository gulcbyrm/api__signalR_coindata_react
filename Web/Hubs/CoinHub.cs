    ...
    public class CoinHub : Hub
    {
       ...
       private List<CurrencyResponse> _lastCoinData;
        ...
        public async Task SendCoinData()
        {
            ...
            {
                i++;
                string conId = GetConnectionID(); // Kullanıcının kimlik bilgisini al
                ...
                if (_lastCoinData != null)
                {
                    ...
                    await Clients.All.SendAsync("ReceiveCoinData", conId, _lastCoinData);
                }
                else
                {
                ...
                await Task.Delay(TimeSpan.FromSeconds(7)); //   saniye bekle
            }
        }
    }
}
